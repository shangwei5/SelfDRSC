...
class ModelSRSCRSG(ModelPlain):
    ...
    def feed_data(self, data, need_H=True):
        ...
        self.dis_encodings = self.dis_encodings.to(self.device)[:, :, :, :, self.diff_patch//2:-self.diff_patch//2, self.diff_patch//2:-self.diff_patch//2]
    ...
    def pad(self, img, ratio=32):
        if len(img.shape) == 5:
            ...
            img = F.pad(img, padding, mode='circular')    # 'replicate'
            ...
            for i in range(n1):
                ...
                pw = ((w - 1) // ratio + 1) * ratio
                padding = (0, pw - w, 0, ph - h)
                img1 = F.pad(img1, padding, mode='circular')   #'replicate'
    ...
    def netG_forward(self, is_train=True):
        ...
        if is_train:
            ...
            assert self.all_time_rsc.size()[1]//2 == self.ext_frames, self.all_time_rsc.size()[1]   #9
            E_list = []
            avg_time = 0
